...
class ProteinGraphDataset(Dataset):
    ...
    def process(self):
        raise NotImplementedError("Use process_mp instead.")
        """Processes structures from files into PyTorch Geometric Data."""
        # Preprocess PDB files
        if self.pdb_transform:
            self.transform_pdbs()
        ...
        for chunk in tqdm(chunks):
            ...
            for i, g in enumerate(graphs):
                try:
                    ...
                    print(f"failed to convert graph {g.name}")
                    continue
...
class OldPhosphositeGraphDataset(ProteinGraphDataset):
    """ 
    Dataset class that assigns node labels (phosphosite vs unmodified) 
    to protein graphs. 
    
    Each graph stores a `y_index` attribute that is used to index into
    the nodes that have labels, and the `y` attribute stores the one-hot
    encoded labels that correspond to the nodes at `y_index`.
    """
    def __init__(
            self, 
            root: str, 
        ...
        self.uniprot_ids = uniprot_ids
        self.root = root
        super().__init__(root, paths, pdb_codes, uniprot_ids, graph_labels, node_labels, chain_selections, graphein_config, graph_format_convertor, graph_transformation_funcs, pdb_transform, transform, pre_transform, pre_filter, num_cores, af_version)
        self.y_label_map = y_label_map
    ...
    def from_index_dict(
        cls,
        index_dict: Dict[Dict[str, Tensor]], 
        **kwargs, 
    ) -> PhosphositeGraphDataset:
        """
        Create a PhosphositeGraphDataset from a dictionary of labels.


        """
        uniprot_ids = list(index_dict.keys())

        dataset = PhosphositeGraphDataset(
        ...
        # TODO: 
        # extra filtering step to make sure that only valid ids are in `modiform_ids` 
        # for future indexing?
